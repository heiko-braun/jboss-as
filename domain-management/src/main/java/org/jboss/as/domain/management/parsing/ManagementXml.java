...
public class ManagementXml {
            ...
            switch (element) {
                case ROLE: {
                    parseScopedRole(reader, address, expectedNs, list, scopedRoleType, Element.SERVER_GROUP,
                            ServerGroupScopedRoleResourceDefinition.BASE_ROLE, ServerGroupScopedRoleResourceDefinition.SERVER_GROUPS);
                    break;
                }
                default: {
                    throw unexpectedElement(reader);
            ...
            switch (element) {
                case ROLE: {
                    parseScopedRole(reader, address, expectedNs, list, scopedRoleType, Element.HOST,
                            HostScopedRolesResourceDefinition.BASE_ROLE, HostScopedRolesResourceDefinition.HOSTS);
                    break;
                }
                default: {
                    throw unexpectedElement(reader);
    ...
    private static void parseScopedRole(XMLExtendedStreamReader reader, ModelNode address, Namespace expectedNs,
                                 ...
                                 SimpleAttributeDefinition baseRoleDefinition, ListAttributeDefinition listDefinition) throws XMLStreamException {
        ...
        for (int i = 0; i < count; i++) {
            ...
            switch (attribute) {
                case NAME:
                    ourAddress.add(scopedRoleType, name);
                    break;
                case BASE_ROLE:
    ...
    public void writeManagement(final XMLExtendedStreamWriter writer, final ModelNode management, final ModelNode accessControl, boolean allowInterfaces)
        ...
        boolean hasHostRoles = accessControl.isDefined() && accessControl.hasDefined(HOST_SCOPED_ROLE);
